...
def feature_generation(V, lob, inflation, seed1):
    ...
    :param LoB.dist: categorical distribution for the allocation of the claims to the 4 lines of business
    :param inflation: growth parameters (per LoB) for the numbers of claims in the 12 accident years
    ...
    random.seed(seed1+3)
    Sigma = pd.DataFrame(read_project('dane')['Simulation.Machine.V1']['Feature.Generation.Parameters']['LoB1and2']['Covariances']['Covariance'])
    ...
    param = pd.DataFrame(read_project('dane')['Simulation.Machine.V1']['Feature.Generation.Parameters']['LoB1and2']['Parameters']['cc'])
    help[:, 0] = np.ceil(f_cc(np.array(help[:, 0]), param.loc[0, 'x'] ,param.loc[1, 'x'], param.loc[2, 'x']))
    translator = pd.DataFrame(read_project('dane')['Simulation.Machine.V1']['Feature.Generation.Parameters']['LoB1and2']['Translators']['cc'])
    ...
    param2 = pd.DataFrame(read_project('dane')['Simulation.Machine.V1']['Feature.Generation.Parameters']['LoB1and2']['Parameters']['age'])
    ...
    param3 = pd.DataFrame(read_project('dane')['Simulation.Machine.V1']['Feature.Generation.Parameters']['LoB1and2']['Parameters']['inj_part'])
    help[:, 3] = np.ceil(f_inj_part(help[:, 3], param3.loc[0, 'x'], param3.loc[1, 'x'], param3.loc[2, 'x']))
    translator2 = pd.DataFrame(read_project('dane')['Simulation.Machine.V1']['Feature.Generation.Parameters']['LoB1and2']['Translators']['inj_part'])
    ...
    random.seed(seed1+4)
    Sigma = pd.DataFrame(read_project('dane')['Simulation.Machine.V1']['Feature.Generation.Parameters']['LoB3and4']['Covariances']['Covariance'])
    help = np.random.multivariate_normal(mean=[0, 0, 0, 0], cov=Sigma, size=len(features[features.LoB>2]))
    help = sp.stats.norm.cdf(help)
    ...
    # Claims code (cc)
    # Source the parameters
    param = pd.DataFrame(read_project('dane')['Simulation.Machine.V1']['Feature.Generation.Parameters']['LoB3and4']['Parameters']['cc'])
    help[:, 0] = np.ceil(f_cc(np.array(help[:, 0]), param.loc[0, 'x'], param.loc[1, 'x'], param.loc[2, 'x']))
    translator = pd.DataFrame(read_project('dane')['Simulation.Machine.V1']['Feature.Generation.Parameters']['LoB3and4']['Translators']['cc'])
    help[:, 0] = np.array(translator.loc[help[:, 0]-1, 'x']).transpose()
    ...
    # Age of the injured (age) 
    param2 = pd.DataFrame(read_project('dane')['Simulation.Machine.V1']['Feature.Generation.Parameters']['LoB3and4']['Parameters']['age'])
    help[np.where(help[:, 2]<=param2.loc[4, 'x'])[0], 2] = np.ceil(f_age1(np.array(help[np.where(help[:, 2]<=param2.loc[4, 'x'])[0], 2]), param2.loc[0, 'x'], param2.loc[1, 'x'], param2.loc[3, 'x']))
    help[np.where(help[:, 2]<=1)[0], 2] = np.ceil(f_age2(np.array(help[np.where(help[:, 2]<=1)[0], 2]), param2.loc[0, 'x'], param2.loc[1, 'x'], param2.loc[2, 'x'], param2.loc[3, 'x']))

    # Injured Part (inj_part)
    param3 = pd.DataFrame(read_project('dane')['Simulation.Machine.V1']['Feature.Generation.Parameters']['LoB3and4']['Parameters']['inj_part'])
    help[:, 3] = np.ceil(f_inj_part(help[:, 3], param3.loc[0, 'x'], param3.loc[1, 'x'], param3.loc[2, 'x']))
    translator2 = pd.DataFrame(read_project('dane')['Simulation.Machine.V1']['Feature.Generation.Parameters']['LoB3and4']['Translators']['inj_part'])
    help[:, 3] = np.array(translator2.loc[help[:, 3]-1, 'x']).transpose()
